...
module.exports = screen => {
  ...
  const hintBar = blessed.Text({
    ...
    height: 1,
    tags: true,
    content: `${ma('+')} append album, ${ma('!')} append all albums, ${ma('space')} load & play selected ${ma('q')} close`
  });

  let loadedAlbums = false;

  box.on('show', () =>  box.setFront() || albums.focus() || command.Albums.FetchForPlaying());
  ...
  albums.key(['q'], () => box.hide() || screen.render());
