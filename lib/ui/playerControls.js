...
module.exports = screen => {
  ...
  const hintBar = blessed.Text({
    ...
    height: 1,
    tags: true,
    content: `${ma('b')} prev ${ma('n')} next ${ma('a')} album ${ma(
      't'
    )} track ${ma('^p')} playlist ${ma('d')} devices ${ma('^q')} quit`
  });

  const iconBar = blessed.Text({
    parent: box,
    bottom: 0,
    height: 1,
    width: 6,
  ...
  const tick = () => {
    ...
    if (playback.progress > playback.duration) {
      ...
      playback.progress = 0;
      commands.Tracks.PlayNext().then(() => fetchCurrent).catch(e => {});
    }
  };
